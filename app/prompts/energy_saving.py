...
2. **小时级过程依据**：完整展示 `expansion_process_table`。它已包含低业务占比、低业务判断、平均休眠时长、零休眠判断和扩展建议；没有数据时如实说明缺失，不以 0 替代缺失值。
3. **两套扩展候选**：完整展示 `expansion_candidate_table`，分别保留“22:00至次日08:00”和“00:00至06:00”两套结果。
4. **两套连续部署结果**：完整展示 `expansion_deployment_table`，直接呈现含扩展时段和仅常规夜间时段的部署小时集合、连续部署时段；不要解释连续时段的推导方式。
5. **参数核查**：当 `param_check.success` 为 false 或 `param_check.is_compliant` 为 null 时，明确写“参数核查暂不可用”，不得作出合规结论；为 true 时说明配置合规；为 false 时展示精简不合规项（参数项、当前值、建议值、影响）。
6. **综合扩展建议**：给出首选方案（含扩展时段的连续部署结果）和备选方案（仅常规夜间时段的连续部署结果）。高负荷或参数不合规时，写成满足前置条件后再实施的建议。
...
### 特殊情况备注
根据 `is_whitelist` 输出节电白名单匹配情况：
- 为 false：明确写“未命中节电白名单”。
- 为 true：展示白名单原因 `whitelist_reason`、生效开始时间 `jd_starttime`、生效结束时间 `jd_endtime`；任一缺失项写“未提供”。并提示修改节能配置前须确认业务影响。
