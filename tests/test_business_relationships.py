...
def test_rru_inventory_defaults_to_cell_day_and_expands_only_when_requested(
    snapshot,
) -> None:
    detail = make_table(
        "nr_report_day_detail",
        {"cgi": "text", "data_date": "date", "traffic": "numeric"},
        grain_keys={"cell_day": ("cgi", "data_date")},
    ).model_copy(update={"default_date_column": "data_date"})
    rru = make_table(
        "sa_nrm_inventoryunitrru",
        {"cgi": "text", "data_date": "date", "serialnumber": "text"},
        default_grain="rru_item",
        grain_keys={"rru_item": ("cgi", "data_date", "serialnumber")},
    ).model_copy(update={"default_date_column": "data_date"})
    relationship = CatalogRelationship(
        name="nr_detail_to_rru",
        left_table=detail.name,
        right_table=rru.name,
        cardinality="one_to_many_latest_snapshot",
        keys=(("cgi", "cgi"),),
        snapshot_column="data_date",
        allowed_grains=("cell_day", "rru_item"),
        detail_grain="rru_item",
    )
    snapshot = snapshot.model_copy(update={
        "tables": {**snapshot.tables, detail.name: detail, rru.name: rru},
        "relationships": {
    ...
    default_plan = BusinessQueryPlan(
        base_table=detail.name,
        tables=[detail.name, rru.name],
        relationships=[relationship.name],
        select=[QueryFieldRef(table=rru.name, field="serialnumber")],
    )
    detail_plan = default_plan.model_copy(update={"result_grain": "rru_item"})

    default_query = validate_query_plan(
        default_plan,
        snapshot,
        Settings(_env_file=None),
    )
    detail_query = validate_query_plan(
        detail_plan,
        snapshot,
        Settings(_env_file=None),
    )

    assert default_query.path.edges[0].preaggregate is True
    assert detail_query.path.edges[0].preaggregate is False
